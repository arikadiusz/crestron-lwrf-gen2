...
namespace crestron_lwrf_gen2
{
    ...
    {
        internal static void lightwaveRfRoomList(HTTPSClientResponseCallback response)
        {
            try
            {
                ...
                HttpRequests.GetAsync(@"https://publicapi.lightwaverf.com/v1/rooms", headers, response);

            }
            catch (Exception exc)
            {
                ErrorLog.Error("Exception in LwrfGen2.lightwaveRfRoomList, Message : {0}", exc.Message);
            }
        }
    }
}
