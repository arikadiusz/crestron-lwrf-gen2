...
namespace crestron_lwrf_gen2
{
    ...
    {
        internal static void lightwaveRfZoneList(HTTPSClientResponseCallback response)
        {
            try
            {
                ...
                HttpRequests.GetAsync(@"https://publicapi.lightwaverf.com/v1/zones", headers, response);

            }
            catch (Exception exc)
            {
                ErrorLog.Error("Exception in LwrfGen2.lightwaveRfZoneList   , Message : {0}", exc.Message);
            }
        }
    }
}
