...
namespace crestron_lwrf_gen2
{
    ...
    {
        internal static void lightwaveRfPatchEventCreate(LightwaveRFFeedbackCreateRootObject eventObj)
        {
            try
            {
                ...
                string response = HttpRequests.Post(@"https://publicapi.lightwaverf.com/v1/events", content, headers);
            }
            catch (Exception exc)
            {
